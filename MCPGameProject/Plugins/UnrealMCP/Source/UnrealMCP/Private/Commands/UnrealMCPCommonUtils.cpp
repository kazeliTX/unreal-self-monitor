...
{
    // Use the provided path if given; otherwise fall back to the default location.
    FString ResolvedPath = AssetPath.IsEmpty()
        ? (TEXT("/Game/Blueprints/") + BlueprintName)
        : AssetPath;
    return LoadObject<UBlueprint>(nullptr, *ResolvedPath);
}
