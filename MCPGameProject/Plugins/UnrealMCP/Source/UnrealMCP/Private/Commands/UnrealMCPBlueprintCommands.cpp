...
{
    Registry.RegisterCommand(TEXT("create_blueprint"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleCreateBlueprint(P); });
    Registry.RegisterCommand(TEXT("add_component_to_blueprint"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleAddComponentToBlueprint(P); });
    Registry.RegisterCommand(TEXT("set_component_property"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleSetComponentProperty(P); });
    Registry.RegisterCommand(TEXT("set_physics_properties"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleSetPhysicsProperties(P); });
    Registry.RegisterCommand(TEXT("compile_blueprint"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleCompileBlueprint(P); });
    Registry.RegisterCommand(TEXT("set_blueprint_property"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleSetBlueprintProperty(P); });
    Registry.RegisterCommand(TEXT("set_static_mesh_properties"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleSetStaticMeshProperties(P); });
    Registry.RegisterCommand(TEXT("set_pawn_properties"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleSetPawnProperties(P); });

    // Blueprint query commands
    Registry.RegisterCommand(TEXT("get_blueprint_variables"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleGetBlueprintVariables(P); });
    Registry.RegisterCommand(TEXT("get_blueprint_functions"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleGetBlueprintFunctions(P); });
    Registry.RegisterCommand(TEXT("get_blueprint_components"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleGetBlueprintComponents(P); });
    Registry.RegisterCommand(TEXT("list_blueprints"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleListBlueprints(P); });
    Registry.RegisterCommand(TEXT("get_blueprint_compile_errors"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleGetBlueprintCompileErrors(P); });

    // Collision commands
    Registry.RegisterCommand(TEXT("set_component_collision_profile"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleSetComponentCollisionProfile(P); });
    Registry.RegisterCommand(TEXT("set_component_collision_enabled"),
        [this](const TSharedPtr<FJsonObject>& P) { return HandleSetComponentCollisionEnabled(P); });
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleGetBlueprintVariables(const TSharedPtr<FJsonObject>& Params)
{
    ...
    for (const FBPVariableDescription& Var : Blueprint->NewVariables)
    {
        ...
        VarObj->SetBoolField(TEXT("exposed"), (Var.PropertyFlags & CPF_Edit) != 0);
    ...
    Result->SetNumberField(TEXT("count"), static_cast<double>(VarArray.Num()));
    return Result;
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleGetBlueprintFunctions(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Event graphs (EventGraph contains event nodes)
    for (UEdGraph* Graph : Blueprint->UbergraphPages)
    {
        if (!Graph) continue;
        TSharedPtr<FJsonObject> FuncObj = MakeShared<FJsonObject>();
        FuncObj->SetStringField(TEXT("name"), Graph->GetName());
        FuncObj->SetStringField(TEXT("type"), TEXT("EventGraph"));
        FuncArray.Add(MakeShared<FJsonValueObject>(FuncObj));
    }

    TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
    Result->SetArrayField(TEXT("functions"), FuncArray);
    Result->SetNumberField(TEXT("count"), static_cast<double>(FuncArray.Num()));
    return Result;
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleGetBlueprintComponents(const TSharedPtr<FJsonObject>& Params)
{
    ...
    Result->SetNumberField(TEXT("count"), static_cast<double>(CompArray.Num()));
    return Result;
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleListBlueprints(const TSharedPtr<FJsonObject>& Params)
{
    ...
    for (const FString& AssetPath : AssetPaths)
    {
        ...
        if (AssetClass == TEXT("Blueprint") || AssetClass == TEXT("AnimBlueprint") || AssetClass == TEXT("WidgetBlueprint"))
        {
            ...
            BPObj->SetStringField(TEXT("path"), AssetPath);
            BPObj->SetStringField(TEXT("name"), AssetData.AssetName.ToString());
            BPObj->SetStringField(TEXT("class"), AssetClass);
    ...
    Result->SetNumberField(TEXT("count"), static_cast<double>(BPArray.Num()));
    return Result;
}
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleGetBlueprintCompileErrors(const TSharedPtr<FJsonObject>& Params)
{
    ...
    // Compile the blueprint to get current status
    FKismetEditorUtilities::CompileBlueprint(Blueprint, EBlueprintCompileOptions::SkipGarbageCollection);

    TArray<TSharedPtr<FJsonValue>> ErrorArray;
    bool bHasErrors = Blueprint->Status == EBlueprintStatus::BS_Error;
    bool bHasWarnings = Blueprint->Status == EBlueprintStatus::BS_UpToDateWithWarnings;

    // Check for compile messages stored in the blueprint
    for (const FBPVariableDescription& Var : Blueprint->NewVariables)
    {
        // Variables are fine, just a placeholder - actual errors come from the compile log
    }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleSetComponentCollisionProfile(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    FString ProfileName;
    if (!Params->TryGetStringField(TEXT("profile_name"), ProfileName) || ProfileName.IsEmpty())
    {
        return FUnrealMCPCommonUtils::CreateErrorResponse(TEXT("Missing 'profile_name' parameter (e.g. 'NoCollision', 'BlockAll', 'OverlapAll', 'Pawn')"));
    }
    ...
    for (USCS_Node* Node : Blueprint->SimpleConstructionScript->GetAllNodes())
    {
        if (!Node || Node->GetVariableName().ToString() != ComponentName) continue;

        UPrimitiveComponent* PrimComp = Cast<UPrimitiveComponent>(Node->ComponentTemplate);
        if (!PrimComp)
        {
            return FUnrealMCPCommonUtils::CreateErrorResponse(
                FString::Printf(TEXT("Component '%s' is not a PrimitiveComponent"), *ComponentName));
        }
...
TSharedPtr<FJsonObject> FUnrealMCPBlueprintCommands::HandleSetComponentCollisionEnabled(const TSharedPtr<FJsonObject>& Params)
{
    FString BlueprintName;
    if (!Params->TryGetStringField(TEXT("blueprint_name"), BlueprintName))
    {
    ...
    for (USCS_Node* Node : Blueprint->SimpleConstructionScript->GetAllNodes())
    {
        ...
        ECollisionEnabled::Type CollType = bEnabled
