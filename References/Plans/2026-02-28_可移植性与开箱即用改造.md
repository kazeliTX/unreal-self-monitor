...
### P1 — 下一步（UE4 兼容核心）
- [ ] 新建 `UnrealMCPCompat.h`（版本宏）
- [ ] Build.cs：IWYU 分支 + BlueprintEditorLibrary 条件引入
- [ ] `UnrealMCPBridge.cpp`：替换 `FAppStyle` → `MCP_STYLE_NAME`
- [ ] `UnrealMCPProjectCommands.cpp`：Enhanced Input 条件编译
- [ ] `.uplugin`：更新最低版本声明
