...
### P0 — 核心价值，优先实现
- [x] Phase C：编辑器菜单开关（改动小，用户价值高）
  - [x] C2: 新增 `UUnrealMCPSettings`（`config=EditorPerProjectUserSettings`，Port + bAutoStartServer）
  - [x] C3: `RegisterMenus()` + `ToggleServer()`（ToolMenus + FUIAction + ToggleButton）
  - [x] C1: Initialize() 改为读配置自启；Deinitialize() 清理菜单注册
...
- [x] UE Editor Tools 菜单出现 "MCP Server" 开关，点击可切换状态
