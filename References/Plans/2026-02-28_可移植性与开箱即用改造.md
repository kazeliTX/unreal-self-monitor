...
## 二、UE4 vs UE5 兼容差异（基于 UE4 参考工程 UE4.24 分析）
...
### P2 — 安装脚本
- [x] `install.py`：核心流程（检测引擎 + 复制插件 + 修补 .uproject）
- [x] `install.py`：源码编译路径发现（UE4 源码编译场景）
- [x] `install.py`：Launcher 注册表路径发现（UE5 场景）
- [x] `install.py`：兼容性报告输出

### P3 — Python 工具
- [x] `project_info_tools.py`：`get_project_info` / `get_engine_install_info` / `check_mcp_compatibility`

---

## 六、验收标准（更新版）

- [ ] 在 UE4 源码编译参考工程（UE4.24.2）上执行 `python install.py` 后，插件正确注入 .uproject
- [ ] 插件在 UE4.24 下编译通过（Enhanced Input 相关代码被条件编译排除）
- [ ] UE Editor Tools 菜单在 UE4 和 UE5 下均能显示 MCP Server 开关
- [ ] `get_project_info` 正确识别 UE4 参考工程为 UE4.24.2 源码编译
