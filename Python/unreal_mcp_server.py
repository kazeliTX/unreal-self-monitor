...
def auto_register_tools(mcp_server: FastMCP) -> None:
    """Discover and register all tool modules under the tools/ package.

    Any module inside tools/ that defines a function matching
    'register_*_tools(mcp)' will be auto-loaded.  Adding a new tool file
    (e.g. tools/animation_tools.py) with a matching register function is
    sufficient — this file does not need to be modified.
    """
    import importlib
    import pkgutil
    ...
    for _, module_name, _ in pkgutil.iter_modules(tools_path):
        ...
        for attr in dir(module):
            if attr.startswith("register_") and attr.endswith("_tools"):
                getattr(module, attr)(mcp_server)
                logger.info(f"Auto-registered: tools.{module_name}.{attr}")
...
def info():
    ...
    ## Diagnostics Tools (Phase 2A)
    ### Visual Perception
    - `get_viewport_camera_info()` - Viewport camera location/rotation/FOV
    - `get_actor_screen_position(name)` - Actor pixel position in viewport
    - `highlight_actor(name)` - Select + focus viewport on actor
    - `take_and_read_screenshot(filepath=None)` - Screenshot + base64 PNG
    - `capture_actor_focused(name, filepath=None)` - Highlight then screenshot
    - `get_scene_overview()` - One-shot scene snapshot (camera + actors + PNG)

    ### Log Analysis
    - `get_log_file_path(project_dir=None)` - Find latest UE log file
    - `read_log_tail(log_path=None, lines=200)` - Read last N log lines
    - `analyze_log_errors(log_path=None, max_errors=50)` - Extract errors/warnings
    - `search_log(pattern, log_path=None, max_results=100)` - Regex search in log
    - `get_log_summary(log_path=None)` - Compact stats + top categories

    ### Compile Management
    - `read_cpp_file(path)` - Read C++ source file (project-relative)
    - `write_cpp_file(path, content)` - Write C++ source file (auto-backup)
    - `read_python_tool(tool_name)` - Read a Python tool module source
    - `write_python_tool(tool_name, content)` - Write Python tool (auto-backup)
    - `reload_python_tool(tool_name)` - Hot-reload Python tool without MCP restart
    - `trigger_hot_reload()` - Trigger LiveCoding compile in editor
    - `get_live_coding_status()` - Check LiveCoding availability
    - `get_engine_path()` - Engine/project paths + UBT script location
    - `run_ubt_build(target_name=None, configuration='Development')` - Full UBT build

    ### System / Lifecycle
    - `check_editor_status()` - 5-dimensional readiness matrix
    - `launch_editor(project_file=None, editor_exe=None)` - Launch editor process
    - `wait_for_editor_ready(timeout_seconds=120)` - Poll until editor is ready
    - `restart_editor(wait_ready=True)` - Terminate + relaunch editor
